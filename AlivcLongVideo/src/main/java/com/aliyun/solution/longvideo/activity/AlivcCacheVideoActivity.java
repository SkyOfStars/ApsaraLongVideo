...
import com.aliyun.solution.longvideo.adapter.AlivcCacheVideoDownloadAdapter;
...
public class AlivcCacheVideoActivity extends AppCompatActivity implements View.OnClickListener, AliyunDownloadInfoListener {
    ...
    public static final String RESULT_WATCHED_NUMBER = "result_watched_number";
    ...
    /**
     * 需要刷新观看历史数量的MediaInfo
     */
    private AliyunDownloadMediaInfo mNeedRefreshMediaInfo;
                ...
                for (AliyunDownloadMediaInfo mediaInfo : aliyunDownloadMediaInfos) {
                    ...
                    if (mediaInfo.getStatus() != AliyunDownloadMediaInfo.Status.Complete) {
                        if (TextUtils.isEmpty(mediaInfo.getTvId())) {
                            ...
                            mAliyunDownloadManager.prepareDownloadLists(mVidSts, mediaInfo);
                        }
                    }
                }
    ...
    private void initView() {
        ...
        mPlayerDownloadAdapter = new AlivcCacheVideoDownloadAdapter(AlivcCacheVideoActivity.this);
    ...
    private void loadDb(final boolean isDownload) {
        ...
        //读取数据库，获取只有一个tvId的视频
        mAliyunDownloadManager.findDatasByDbTv(new LoadDbDatasListener() {
            @Override
            public void onLoadSuccess(List<AliyunDownloadMediaInfo> dataList) {
                ...
                mDownloadMediaInfos.addAll(dataList);

                mPlayerDownloadAdapter.setData(mDownloadMediaInfos);

                if(mDownloadMediaInfos.size() > 0){
                    if (isDownload) {
    ...
    private void initData() {
        ...
        mPlayerDownloadAdapter.setOnItemClickListener(new AlivcCacheVideoDownloadAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(AlivcCacheVideoDownloadAdapter adapter, View view, int position) {
        ...
        //预览图点击
        mPlayerDownloadAdapter.setOnItemChildClickListener(new AlivcCacheVideoDownloadAdapter.OnItemChildClickListener() {
            @Override
            public void onItemChildClick(AlivcCacheVideoDownloadAdapter adapter, View view, int position) {
                AliyunDownloadMediaInfo mediaInfo = adapter.getDatas().get(position);
                if (!mIsEditing) {
                    if (view.getId() == R.id.alivc_iv_preview) {
                        mStartOtherActivity = true;
                        if (!TextUtils.isEmpty(mediaInfo.getTvId())) {
                            //跳到子页面，并且根据tvId查询数据
                            mNeedRefreshMediaInfo = mediaInfo;
                            Intent intent = new Intent(AlivcCacheVideoActivity.this, AlivcChildCacheVideoActivity.class);
                            intent.putExtra(AlivcChildCacheVideoActivity.KEY_ALIVC_CACHE_TV_ID, mediaInfo.getTvId());
                            startActivityForResult(intent,REQUEST_CODE);
                        } else {
                            AlivcPlayerActivity.startAlivcPlayerActivity(AlivcCacheVideoActivity.this, ObjectToLongVideo.downloadMediaInfoToLongVideo(mediaInfo));
                            //如果是未观看的视频,点击后设置已观看
                        ...
                        if (!mIsOperatorDownld && !mIsNetWorkconnect) {
                            FixedToastUtils.show(AlivcCacheVideoActivity.this, getResources().getString(R.string.alivc_longvideo_cache_toast_4g));
                            return;
                        }
                        if (mediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.Prepare || mediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.Stop || mediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.Error || mediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.Wait) {
                            if (mCheckResult) {
                                ...
                                PermissionUtils.requestPermissions(AlivcCacheVideoActivity.this, permission, PERMISSION_REQUEST_CODE);
                            }
                        } else {
    ...
    private void calculationTotal() {
        ...
        if (mIsEditing) {
            ...
            //恢复编辑前的状态
            if (totalNumber == 0) {
                mTvDelete.setTextColor(ContextCompat.getColor(AlivcCacheVideoActivity.this, R.color.alivc_common_font_gray_333333));
                mTvDelete.setText(getResources().getString(R.string.alivc_longvideo_cachevideo_delete));
            } else {
                mTvDelete.setTextColor(ContextCompat.getColor(AlivcCacheVideoActivity.this, R.color.alivc_common_bg_red_darker));
    ...
    private void deleteItem() {
        ...
        //如果是剧集，要删除剧集所有，先从数据库查询tvId，在删除
        for (int i = 0; i < mDownloadMediaInfos.size(); i++) {
            if (mDownloadMediaInfos.get(i).isSelected()) {
                //剧集,如果是第二层页面，可以删除单个
                if (!TextUtils.isEmpty(mDownloadMediaInfos.get(i).getTvId())) {
                    //获取所有系列删除
                    mAliyunDownloadManager.getDownloadMediaInfoWithTvId(mDownloadMediaInfos.get(i).getTvId(), new LoadDbTvListDatasListenerr() {
                        @Override
                        public void onLoadTvListSuccess(List<AliyunDownloadMediaInfo> aliyunDownloadMediaInfos) {
                            for (AliyunDownloadMediaInfo mediaInfo : aliyunDownloadMediaInfos) {
                                mAliyunDownloadManager.deleteFile(mediaInfo);
    ...
    /**
     * 视频缓存页入口
     * 不建议传递tvId，因为会直接进入系列页面
     */
    public static void startJump(Context context) {
        Intent intent = new Intent(context, AlivcCacheVideoActivity.class);
    ...
    public void onPrepared(List<AliyunDownloadMediaInfo> infos) {
        ...
        for (int i = 0; i < infos.size(); i++) {
            for (int j = 0; j < mDownloadMediaInfos.size(); j++) {
                ...
                if (infos.get(i).getVid().equals(mDownloadMediaInfos.get(j).getVid()) && infos.get(i).getQuality().equals(mDownloadMediaInfos.get(j).getQuality())) {
                    Log.i("scar", "onPrepared:命中下载 " + infos.get(i).getVid() + "___" + infos.get(i).getQuality() + "__" + infos.get(i).getStatus());
        ...
        //删除原本的数据，插入新数据
        if (isHit) {
            Log.i("scar", "onPrepared:未命中下载 " + infos.get(0).getVid() + "___" + infos.get(0).getQuality() + "__" + infos.get(0).getStatus());
    ...
    public void onStart(AliyunDownloadMediaInfo info) {
        ...
        Log.i("scar", "onStart更新 : " + info.getTvId() + "_vid__" + info.getVid() + "__" + info.getStatus() + "__" + info.getQuality());
        //开始下载状态更新
    ...
    public void onProgress(AliyunDownloadMediaInfo info, int percent) {
        ...
        //下载进度条
        // Log.d("scar", "title " + info.getTitle() + " " + info.getVid() + "___" + info.getStatus() + "-----------||||||" + percent + "___quantity :" + info.getQuality());
        //更新对应数据的进度条
        mPlayerDownloadAdapter.updateData(info);
    }
    ...
    public void onStop(AliyunDownloadMediaInfo info) {
        ...
        Log.i("scar", "onStop暂停 : " + info.getTvId() + "_vid__" + info.getVid() + "__" + info.getStatus() + "__" + info.getQuality());
        mPlayerDownloadAdapter.updateData(info);
    ...
    public void onCompletion(AliyunDownloadMediaInfo info) {
        //下载完成
        Log.d(TAG, info.getVid() + "-----------onCompletion");
    ...
    public void onError(AliyunDownloadMediaInfo info, ErrorCode code, String msg, String requestId) {
        ...
        Log.e("scar", "onError: " + info.getTitle() + "__" + msg);
        ToastUtils.show(AlivcCacheVideoActivity.this, msg);
        mPlayerDownloadAdapter.updateData(info);
    }
    ...
    public void onDelete(AliyunDownloadMediaInfo info) {
        //刷新数据
        Log.i("scar", "onDelete: " + info.getVid());
        reFreshData(info);
        FixedToastUtils.show(AlivcCacheVideoActivity.this, getResources().getString(R.string.alivc_longvideo_cachevideo_delected_cachevideo));
    ...
    private void onNetDisconnected() {
        FixedToastUtils.show(AlivcCacheVideoActivity.this, getResources().getString(R.string.aliyun_net_error));
        //网络断开。
    ...
    private static class MyNetChangeListener implements NetWatchdog.NetChangeListener {
        ...
        public MyNetChangeListener(AlivcCacheVideoActivity cacheVideoActivity) {
            viewWeakReference = new WeakReference<AlivcCacheVideoActivity>(cacheVideoActivity);
        }

        @Override
        public void onWifiTo4G() {
            AlivcCacheVideoActivity aliyunVodPlayerView = viewWeakReference.get();
        ...
        public void on4GToWifi() {
            AlivcCacheVideoActivity cacheVideoActivity = viewWeakReference.get();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == REQUEST_CODE){
            if(resultCode == Activity.RESULT_OK && data != null){
                int resultNumber = data.getIntExtra(RESULT_WATCHED_NUMBER, mNeedRefreshMediaInfo.getWatchNumber());
                mNeedRefreshMediaInfo.setWatchNumber(mNeedRefreshMediaInfo.getWatchNumber() + resultNumber);
                if(mPlayerDownloadAdapter != null){
                    mPlayerDownloadAdapter.notifyItemChanged(mDownloadMediaInfos.indexOf(mNeedRefreshMediaInfo));
                }
            }
        }
    }
